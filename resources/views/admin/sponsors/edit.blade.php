            ...
            <form action="{{ route('admin.sponsors.update', $sponsor) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" 
                           id="name" name="name" value="{{ old('name', $sponsor->name) }}" 
                ...
                <div class="mb-3">
                    ...
                    @if($sponsor->logo)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $sponsor->logo) }}" alt="{{ $sponsor->name }}" class="img-thumbnail" style="max-width: 200px;">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                        ...
                        <option value="platinum" {{ old('type', $sponsor->type) === 'platinum' ? 'selected' : '' }}>Platinum</option>
                        <option value="gold" {{ old('type', $sponsor->type) === 'gold' ? 'selected' : '' }}>Gold</option>
                        <option value="silver" {{ old('type', $sponsor->type) === 'silver' ? 'selected' : '' }}>Silver</option>
                        <option value="bronze" {{ old('type', $sponsor->type) === 'bronze' ? 'selected' : '' }}>Bronze</option>
                        <option value="media_partner" {{ old('type', $sponsor->type) === 'media_partner' ? 'selected' : '' }}>Media Partner</option>
                    </select>
                    @error('type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('order') is-invalid @enderror" 
                           id="order" name="order" value="{{ old('order', $sponsor->order) }}" 
                           min="0" required>
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-save"></i> Update Sponsor
                    </button>
