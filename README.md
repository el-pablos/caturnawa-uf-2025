...
The **start.sh** script provides a comprehensive, interactive setup experience that automatically detects your platform and installs all required dependencies.
...
- Identifies available package managers (apt, dnf, brew, choco, etc.)
- Checks existing installations of required software

**📦 Dependency Management:**
- Installs PHP 8.2+ with all required extensions
- Sets up Composer for PHP package management
- Installs Node.js 16+ for frontend asset compilation
- Configures MySQL/MariaDB database server (optional)

**🛠️ Project Configuration:**
- Creates and configures `.env` file from template
- Pre-configures Midtrans payment gateway settings
- Sets up database connection parameters
- Configures mail settings (MailHog for development)
- Generates application keys and security tokens
...
1. **Laravel Only**: Basic PHP development server
2. **Laravel + Vite**: Full development environment with hot-reload
3. **Production Mode**: Optimized build for production testing
...
For production environments, use the automated setup with production optimizations:
...
# During setup, choose production options when prompted
# The script will automatically optimize for production
```
...
The start.sh script pre-configures most settings, but you can customize:
...
# Database (auto-configured by start.sh)
DB_CONNECTION=mysql
DB_HOST=127.0.0.1
DB_PORT=3306
...
# Midtrans Payment Gateway (pre-configured for sandbox)
MIDTRANS_SERVER_KEY=SB-Mid-server-FYFxmeHpL8bGQmSbeGQ7_3EV
MIDTRANS_CLIENT_KEY=SB-Mid-client-zcodFaPnVFzF2xRc
MIDTRANS_IS_PRODUCTION=false
MIDTRANS_IS_SANITIZED=true
MIDTRANS_IS_3DS=true

# Mail Configuration (auto-configured for development)
MAIL_MAILER=smtp
MAIL_HOST=mailpit
MAIL_PORT=1025
MAIL_USERNAME=
...
For production deployments, configure your web server:
...
For production deployment, the start.sh script can be configured for production mode:
...
# - Optimized composer install (--no-dev --optimize-autoloader)
...
The start.sh script automatically configures security settings:

```php
// config/security.php (auto-generated)
return [
    'csrf' => [
        'enabled' => true,
...
graph LR
    ...
    subgraph "Build Optimizations"
        D1[Asset Compilation]
        D2[Code Minification]
        D3[Cache Warming]
        D4[Route Caching]
    end
```
...
The application implements comprehensive caching automatically configured by start.sh:
...
# Re-run start.sh for database configuration
./start.sh
```
...
# Reconfigure Midtrans settings
./start.sh  # Choose to update configuration
```
...
<details>
<summary><strong>🔧 start.sh Script Issues</strong></summary>
...
./start.sh --component=dependencies  # Install only dependencies
./start.sh --component=environment   # Configure only environment
./start.sh --component=database      # Setup only database
```
...
Use the start.sh script for consistent development environment:
...
# During setup, choose development mode for:
# - Debug mode enabled
# - Development dependencies included
# - Hot-reload asset compilation
...
Get started in minutes with our interactive setup script:
...
**✨ Key Features**
- 🏆 Multi-competition management system
...
**Made with ❤️ for UNAS FEST 2025**

*Empowering competitions with modern technology*
